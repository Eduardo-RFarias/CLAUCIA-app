...
class SampleService {
  ...
  Future<List<Sample>> getAllSamples() async {
    ...
    final samples = List<Sample>.from(_mockSamples);
    samples.sort((a, b) => b.date.compareTo(a.date));
  ...
  // Get sample by ID
  Future<Sample?> getSampleById(int sampleId) async {
    await Future.delayed(const Duration(milliseconds: 300));

    try {
      return _mockSamples.firstWhere((sample) => sample.id == sampleId);
    } catch (e) {
      return null;
    }
  }
